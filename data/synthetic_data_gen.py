...
def create_subset(
    df: pd.DataFrame,
    target_circuit_layers: int,
    tolerance: float,
    num_circuits: int
) -> Tuple[pd.DataFrame, Set[int]]:
    current_circuit_layers = 0
    while not (
        current_circuit_layers >= target_circuit_layers - tolerance and
        current_circuit_layers <= target_circuit_layers + tolerance
    ):
        subset_indices = np.random.choice(
            list(df.index), size=min(num_circuits, len(df.index)),
            replace=False
        )
...
def create_subsets_data(num_subsets: int, num_circuits: int) -> pd.DataFrame:
    average_circuit_layers = df_original["original_circuit_layers"].mean()
    tolerance = average_circuit_layers * 0.1

    all_subsets = []
    for i in range(num_subsets):
        subset = create_subset(
            df_original, average_circuit_layers, tolerance, num_circuits
        )
        subset["subset"] = i+1
...
def expand_subsets(df: pd.DataFrame, num_existing_subsets: int, num_new_subsets: int) -> pd.DataFrame:
    ...
    for i in range(1, num_new_subsets + 1):
        ...
        mixed_subset = shuffle(
            pd.concat([
                grouped.get_group(subset).sample(frac=1, replace=False).reset_index(drop=True)
...
if __name__=="__main__":
    ...
    df_train, df_test = train_test_split(df_final, test_size=0.2, stratify=df_final["subset"], random_state=12)
    df_traim = df_train.sort_values(by="subset")
    df_test = df_test.sort_values(by="subset")
