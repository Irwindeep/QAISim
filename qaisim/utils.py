...
class Dataset:
        ...
        for idx, row in self.df.iterrows():
            row_data = {
                "original": QTaskParams(
                    num_qubits=row["original_num_qubits"],
                    circuit_layers=row["original_circuit_layers"],
                    gate_counts=row["original_gate_counts"],
                ),
                "ibm127": QTaskParams(
                    num_qubits=row["ibm127_num_qubits"],
                    circuit_layers=row["ibm127_circuit_layers"],
                    gate_counts=row["ibm127_gate_counts"],
                ),
                "ibm133": QTaskParams(
                    num_qubits=row["ibm133_num_qubits"],
                    circuit_layers=row["ibm133_circuit_layers"],
                    gate_counts=row["ibm133_gate_counts"],
                ),
                "ibm156": QTaskParams(
                    num_qubits=row["ibm156_num_qubits"],
                    circuit_layers=row["ibm156_circuit_layers"],
    ...
    def random_qtasks(self, num_qtasks):
        ...
        return {
            key: value
            for key, value in random.sample(list(self.data.items()), num_qtasks)
        }
